...
async function startFullStackDevServer(): Promise<string> {
  if (testAppServer && serverBaseUrl) return serverBaseUrl;
...
async function stopFullStackDevServer(): Promise<void> {
  if (testViteServer) await testViteServer.close();
  if (testAppServer) await new Promise<void>((resolve) => testAppServer!.close(() => resolve()));
}
...
async function navigateAndScreenshot(page: Page, navId: string, screenshotPath: string, isFullPage: boolean = true) {
  await page.evaluate((id) => window.dispatchEvent(new CustomEvent('navigate', { detail: id })), navId);
  await page.waitForTimeout(800);
  await page.screenshot({ path: screenshotPath, fullPage: isFullPage });
}

async function runScenario(browser: Browser, baseUrl: string, scenarioId: string, uid: string) {
  const isPopulated = scenarioId === 'populated-household';
  const screenshotsDir = path.resolve(`test-results/ui-screenshots/${scenarioId}`);
  ...
  await authContext.addInitScript(`
    window.__PLAYWRIGHT_TEST_USER__ = {
      uid: '${uid}',
      email: '${uid}@housemind.local',
      displayName: '${isPopulated ? 'Populated User' : 'Empty User'}',
      photoURL: 'https://placekitten.com/150/150',
      testToken: 'test-token-${uid}'
    };
    window.__PLAYWRIGHT_TEST_USER__.getIdToken = function() { return Promise.resolve('test-token-${uid}'); };
  `);
  ...
  await navigateAndScreenshot(page, 'dashboard', path.join(screenshotsDir, 'temp-dashboard.png'));
...
async function main() {
  ...
  console.log('Running Scenario A: Empty Household');
  await runScenario(browser, baseUrl, 'empty-household', 'empty-user');

  console.log('Running Scenario B: Populated Household');
  await runScenario(browser, baseUrl, 'populated-household', 'test-user-e2e'); // 'test-user-e2e' has populated fixtures

  await browser.close();
  await stopFullStackDevServer();

  // Validate screenshot counts
  const emptyCount = fs.readdirSync('test-results/ui-screenshots/empty-household').filter(f => f.endsWith('.png')).length;
  const popCount = fs.readdirSync('test-results/ui-screenshots/populated-household').filter(f => f.endsWith('.png')).length;
  const total = emptyCount + popCount;

  console.log('--- AUDIT RESULTS ---');
  console.log(`Expected minimum screenshots: 64`);
  console.log(`Actual screenshots generated: ${total} (Empty: ${emptyCount}, Populated: ${popCount})`);

  if (total < 64) {
    console.error('AUDIT STATUS: INCOMPLETE');
  } else {
    console.log('AUDIT STATUS: COMPLETE');
  }
}
